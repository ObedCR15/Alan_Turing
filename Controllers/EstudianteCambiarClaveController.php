<?php
require_once "../Models/EstudianteCambiarClaveModel.php";

class EstudianteCambiarClaveController {
    private $model;
    private $error = '';
    private $mensaje = '';

    public function __construct(PDO $pdo) {
        $this->model = new EstudianteCambiarClaveModel($pdo);
    }

    public function cambiarClave(array $postData, int $idEstudiante) {
        $this->error = '';
        $this->mensaje = '';
        ...
        $claveHash = $this->model->obtenerClaveActual($idEstudiante);
        if (!$claveHash) {
        ...
        $actualizado = $this->model->actualizarClave($idEstudiante, $nuevoHash);
